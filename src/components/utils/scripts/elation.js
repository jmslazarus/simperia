...
elation.extend('require.batch', function(type, webroot) {
  ...
  this.pushqueue = function(module) {
    ...
    if (!this.isfulfilled(module) && !this.ispending(module)) {
      ...
      if (elation.env.isBrowser) {
        // browser
        console.log('loading eation module: ', this.type, module, module.replace(/\./g, type === 'js' ? '/scripts/' : '/css/'));
        elation.file.get(this.type, this.webroot + '/' + module.replace(/\./g, type === 'js' ? '/scripts/' : '/css/') + '.' + this.type, elation.bind(this, function(ev) { this.finished(module); }));
  ...
  this.fulfill = function(modules, callback) {
    console.log('FULFILL', modules, typeof callback);
