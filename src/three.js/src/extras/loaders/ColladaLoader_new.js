 ...
 * modified : 6ert: function createSceneGraph	/	http://www.weberlebt.de
 */
...
THREE.ColladaLoader = function () {
	...
	function setupSkinningMatrices ( bones, skin ) {
		...
		for ( var i = 0; i < bones.length; i ++ ) {
			...
			if ( found >= 0 ) {
				...
				bone.skinningMatrix.multiply(bone.world, inv); // (IBMi * JMi) IBMi: Inverse bind-pose matrix of joint i | JMi: Transformation matrix of joint i
				bone.weights = [];
				for ( var j = 0; j < skin.weights.length; j ++ ) {
					for (var k = 0; k < skin.weights[ j ].length; k ++) {
						var w = skin.weights[ j ][ k ];
						if ( w.joint == found ) {
							bone.weights.push( w );//2dokoi: ?
						}
					}
				}
			} else {
	...
	function applySkin ( geometry, instanceCtrl, frame ) {
		...
		var instanceMaterial = instanceCtrl.instance_material;//donekoi: nur so
		// move vertices to bind shape ( v * BSM )
		...
		for ( frame = 0; frame < animationBounds.frames; frame ++ ) {
			...
			// process the frame and setup the rig with a fresh transform,
			// possibly from the bone's animation channel(s)
			...
			switch ( controller.type ) {
				case 'skin':
					if ( geometries[ controller.skin.source ] ) {
						...
						inst_geom.instance_material = node.controllers[ i ].instance_material;//alle sub/materials
		...
		for ( i = 0; i < node.geometries.length; i ++ ) {
			...
			if ( geometry ) {
				...
				if ( instance_materials ) {
					for ( j = 0; j < instance_materials.length; j ++ ) {//donekoi: keine animation bei length > 1 
						var inst_material = instance_materials[j];
						var effect_id = materials[inst_material.target].instance_effect.url;
						var shader = effects[effect_id].shader;
						used_materials[inst_material.symbol] = shader.material;	//assoziativ - 
						used_matarray[ j ] = used_materials[inst_material.symbol];////donekoi: Array[18] mit used_materials Objekten
...
//				if ( num_materials > 1 ) {
				for ( j = 0; j < geom.faces.length; j ++ ) {
					var face = geom.faces[ j ];//face: THREE.Face3 - serialisiert die Anzahl der SUM(count) von <triangles material="*" count="x"> aus <library_geometries>
					face.materials = [ used_materials[ face.daeMaterial ] ];
					//2dokoi: geom.faceUvs[ k ].push in ca ln 1800 hinzugefuegt: da bisher keine faceUvs - rückgängig machen: geom.faceUvs[ k ].push( [ uv[0], uv[1], uv[2] ] );//2dokoi
				}
				if ( skinController !== undefined) {					
					//donekoi: alles ok - wichtig: material loops
					applySkin( geom, skinController );
					for ( var n = 0; n < num_materials; n++) {
						material = new THREE.MeshFaceMaterial();
						material.morphTargets = true;
						//obwohl nicht im skinned mesh verwendet, ist folgendes wichtig!
						used_matarray[ n ].morphTargets = true;//Animation einschalten
						//used_matarray[ n ].morphTargets = false;//Animation ausschalten - skin pose Modus - Modellierungs Grundposition (debug)
					}						
	...
	Mesh.prototype.parse = function( element ) { //localName, tagName = "mesh"
		this.primitives = [];
		var i, j;
		for ( i = 0; i < element.childNodes.length; i ++ ) {
			var child = element.childNodes[ i ];
			...
			switch ( child.nodeName ) {
				case 'source':
					_source( child );//* Beschreibung von <source> element siehe * Klasse Source und Source.parse
					break;
				case 'vertices':
		...
		for ( i = 0, j = 0; i < vertexData.length; i += 3, j ++ ) {//2dokoi: mit tangents, binormals ?
		...
		}//2dokoi: Fehler tritt beim ersten Durchlauf der 2.Geometrie auf
	...
	Mesh.prototype.handlePrimitive = function( primitive, geom, vertex_store ) {
		...
		while ( i < p.length ) {
			...
			for ( j = 0; j < vcount; j ++ ) {// 3
				for ( k = 0; k < inputs.length; k ++ ) { // 3
					input = inputs[ k ];
					source = sources[ input.source ];
					
					//2dokoi: indexberechnung 
					index = p[ i + ( j * inputs.length ) + input.offset ]; 
					numParams = source.accessor.params.length;// 3
					...
					switch ( input.semantic ) {
						case 'VERTEX':
							...
							var hash = _hash_vector3( geom.vertices[ index ].position );//2dokoi: Uncaught TypeError: Cannot read property 'position' of undefined
			...
			for ( k = 0; k < texture_sets.length; k ++ ) {
				...
				geom.faceVertexUvs[ k ].push( [ uv[0], uv[1], uv[2] ] );//2dokoi: baked texture - Uncaught TypeError: Cannot call method 'push' of undefined
				geom.faceUvs[ k ].push( [ uv[0], uv[1], uv[2] ] );//2dokoi: eventuell rueckgaengig !
	...
	/**
	 * Klasse Triangles
	 * aus <triangles> element, das die triangle (dreiecks) struktur beschreibt
	 * bsp:
	 * <triangles count="61250" material="Bounce_on_Deforming_Surface_E1_ME_MAT">
	 *   <input offset="0" semantic="VERTEX" source="#Terrain_E1_MESH_0_REF_1_lib_vertices"/>
	 *   <p>176 0 177 1 177 0 177 1 178 2 178 1 178 2 179 3 179 2 179 3 180 4 180 3 180 4 .....
	 */
	...
	/**
	 * Klasse Vertices
	 * beschreibt das <vertices> element mit den Vertex (Scheitelpunkt) Infos
	 * semantics im <input> sind POSITION NORMAL TEXCOORD 
	 * 2dokoi: was ist mit TANGENT BINORMAL
	 * die streams werden über Namensreferenz in <source> angegeben
	 * zb: 
	 * <vertices id="Terrain_E1_MESH_0_REF_1_lib_vertices">
	 *   <input semantic="NORMAL" source="#Terrain_E1_MESH_0_REF_1_lib_normals"/> 
	 */
	function Vertices() {
		this.input = {};
	};
	...
	/**
	 * Klasse Source 
	 * erzeugt aus dem <source> element (eine Beschreibung des data streams)
	 * besteht im Fall von <geometry> <mesh> aus <float_array> mit Werten 
	 * und einem <accessor> der den data stream beschreibt
	 * hier für position, normal, texcoords, tangent, binormal mit den Angaben:
	 * count, Source, Stride, <param>
	 * zb:
	 * <source id="Terrain_E1_MESH_0_REF_1_lib_binormals" name="binormal">
	 *   <float_array id="Terrain_E1_MESH_0_REF_1_lib_binormals_array" count="92928">0 0.981328 0.192343....
	 *     <technique_common>
	 *       <accessor count="30976" source="#Terrain_E1_MESH_0_REF_1_lib_binormals_array" stride="3">
	...
	/**
	 * Beschreibung von <source> element siehe * Klasse Source und Source.parse
	 */
